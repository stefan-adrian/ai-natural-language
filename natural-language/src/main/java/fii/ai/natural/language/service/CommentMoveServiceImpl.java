    ...
    private void decorateWithMistakeComment(MovesTree movesTree, int indexOfMove) {
        ...
        if (bestMoveVariant != null) {
            ...
            if (bestMoveVariant.getScore() - mainVariant.getScore() > ScoreInfo.getMediumImpactMove()) {
                comment = "At this move was made a big mistake. With moves from a algorithm you could get better results in the the next "+bestMoveVariant.getMoves().size()+" moves. ";
            } else if (bestMoveVariant.getScore() - mainVariant.getScore() > ScoreInfo.getSmallImpactMove()) {
                comment = "At this move was made a mistake. With moves from a algorithm you could get better results in the next "+bestMoveVariant.getMoves().size()+" moves. ";
            }
            if (comment != null) {
