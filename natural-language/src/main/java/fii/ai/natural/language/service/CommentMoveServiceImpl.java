    ...
    private void decorateWithMistakeComment(MovesTree movesTree, int indexOfMove) {
        ...
        if (bestMoveVariant != null) {
            ...
            if (bestMoveVariant.getScore() - mainVariant.getScore() > ScoreInfo.getMediumImpactMove()) {
                comment = "At this move was made a big mistake. With the best moves from a variant this could have happened in the next "+bestMoveVariant.getMoves().size()+" moves. ";
            } else if (bestMoveVariant.getScore() - mainVariant.getScore() > ScoreInfo.getSmallImpactMove()) {
                comment = "At this move was made a mistake. With the best moves from a variant this could have happened in the next "+bestMoveVariant.getMoves().size()+" moves. ";
            }
            if (comment != null) {
